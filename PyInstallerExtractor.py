...
class PyInstArchive:
    ...
    def checkFile(self):
        ...
        self.fPtr.seek (self.fileSize - 2202, os.SEEK_SET)
        magicFromFile = self.fPtr.read(len(self.MAGIC))
    ...
    def getCArchiveInfo(self):
        ...
        try:
            self.fPtr.seek (0, 2)
            filelen = self.fPtr.tell ()
            self.fPtr.seek (max(0,self.fileSize - 4096), os.SEEK_SET)
            searchpos = self.fPtr.tell()
            buf = self.fPtr.read (min (filelen, 4096))
            pos = buf.rfind (self.MAGIC)
            if pos == -1:
                print('[*] Error : The file is not a pyinstaller archive')
                return False
            filelen = searchpos + pos + _cookie_size
            (magic, lengthofPackage, toc, tocLen, self.pyver, pylib_name) = struct.unpack (
                _cookie_format, buf[pos:pos + _cookie_size])
        except:
         print('[*] Error : The file is not a pyinstaller archive')
         return False
        if magic != self.MAGIC:
        ...
        self.overlayPos = filelen - lengthofPackage
    ...
    def extractFiles(self):
        ...
        for entry in self.tocList:
            ...
            if entry.typeCmprsData != b's' and entry.typeCmprsData!=b'z':
                continue
            self.fPtr.seek(self.overlayPos+entry.position, os.SEEK_SET)
